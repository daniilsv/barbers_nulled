...
class UserMainPageView extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return GetBuilder<UserMainPageViewModel>(
      init: Get.put(UserMainPageViewModel()),
      builder: (model) {
        return Scaffold(
          appBar: AppBar(
            title: const Text('Каталог мастеров', style: BRStyle.black14),
            actions: [FilterButton().paddingOnly(right: 24)],
            centerTitle: true,
                                  ...
                                  return Column(
                                    children: <Widget>[
                                      BarberCard(
                                        ...
                                        onTapCall: () => model.call(model.barbers[index]),
                                      ),
                                    ],
                                  );
                                },
          ...
          ).withBackground().withBusyStack(model),
        ).unfocusOnTap();
      },
    );
  }
